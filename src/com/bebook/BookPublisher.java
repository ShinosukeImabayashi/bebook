...
public class BookPublisher extends AsyncTaskLoader <BookPublisher> {
...
public BookPublisher(Context context, String url) {
		super(context);
		xmlurl = url;
	}
...
public void setBookPublisherData () {
...
@Override
public BookPublisher loadInBackground() {
   	// 全書籍データを管理
	Map BookPublisherData= new HashMap ();
	// １書籍データを管理
	...
	// 書籍データ取得
	try {
        Iterator it = BookPublisherData.keySet().iterator();
        while (it.hasNext()) {
            Object o = it.next();
            if (o != null) {
            		Log.d("bookData", o.toString() + "---" + ((Map) BookPublisherData.get(o)).get("id"));
            		Log.d("bookData", o.toString() + "---" + ((Map) BookPublisherData.get(o)).get("name"));
            		ArrayList list = (ArrayList)  ((Map) BookPublisherData.get(o)).get("contentsText");
